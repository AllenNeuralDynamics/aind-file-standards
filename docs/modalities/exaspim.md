...
The raw IMS files are converted to OME-ZARR during upload and the original IMS files are not preserved.
...
Processed assets consist of subfolders for each component process. A complete asset must include folders `tile_alignment`, `fusion`,  `ccf_alignment`, and `soma_detection`.
...
Transformations are saved in `mat` (affine) or `nii.gz` formats (warp), as expected by ANTS.

Transformed image volumes and annotation label volumes must be saved in `zarr` format. 
An additional copy may be saved in `nii.gz` format.
...
Results must be saved to `soma_locations.csv`, containing soma locations in specimen and CCF space in columns `xyz_raw` and `xyz_ccf_auto`.
